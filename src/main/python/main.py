...
if __name__ == '__main__':
    ...
    dateiAITrainingWaitingDialog = appctxt.get_resource("AITrainWaitingDialog.ui")
    aiWaitingDialog = uic.loadUi(dateiAITrainingWaitingDialog)
    ...
    def uiSetup():  # alles was beim Laden passieren soll
        ...
        # Tab: AI-Training: show or hide UI-controls for AI-Training settings, depending on checked-State
        # of Checkbox checkbox_resume_training:
        show_UI_training(mW.checkBox_resume_training.isChecked())
        # initialize lineEdit_folder_ai with AI_Model_Pfad:
        mW.lineEdit_folder_ai.setText(DATA["AI_Model_Pfad"])
        # initialize labels
        configure_AI_description_labels()
    ...
    ### Tab AI-Training
    def mW_button_folder_ai():
        # show folder browser dialog
        if DATA["AI_Model_Pfad"] == "" or DATA["AI_Model_Pfad"] == " " or not os.path.exists(DATA["AI_Model_Pfad"]):
            dirName = widgets.QFileDialog.getExistingDirectory(directory = core.QStandardPaths.
                writableLocation(core.QStandardPaths.DocumentsLocation))
        else:
            dirName = widgets.QFileDialog.getExistingDirectory(directory = DATA["AI_Model_Pfad"])

        # update settings (must be done before updating UI)
        DATA["AI_Model_Pfad"] = dirName

        # Update UI
        mW.lineEdit_folder_ai.setText(dirName)
        configure_AI_description_labels()

        # write settings to disk
        Speichern.Write_json(DATA)
        return
    
    def mW_checkBox_resume_training():
        if mW.checkBox_resume_training.isChecked() == True:
            show_UI_training(True)
        else:
            show_UI_training(False)
        return

    # show or hide UI-controls for AI-training settings
    def show_UI_training(showUI):
        mW.label_descr_training.setVisible(showUI)
        mW.label_folder_train_img.setVisible(showUI)
        mW.lineEdit_folder_train_img.setVisible(showUI)
        mW.button_folder_train_img.setVisible(showUI)
        mW.label_mask_num.setVisible(showUI)
        mW.lineEdit_mask_num.setVisible(showUI)
        mW.label_num_epoch.setVisible(showUI)
        mW.lineEdit_num_epoch.setVisible(showUI)
        mW.label_batch_size.setVisible(showUI)
        mW.lineEdit_batch_size.setVisible(showUI)
        mW.button_start_training.setVisible(showUI)
        return

    def configure_AI_description_labels():
        if(getLatestAIModelName() == ''):
            mW.label_descr_resolution.setVisible(False)
            mW.label_descr_no_model_found.setVisible(True)
            if(mW.lineEdit_folder_ai.text() == ''):
                mW.label_descr_no_model_found.setVisible(False)
            else:
                mW.label_descr_no_model_found.setVisible(True)
        else:
            mW.label_descr_resolution.setVisible(True)
            mW.label_descr_no_model_found.setVisible(False)

            # try to get size of AI-model and adjust text
            size = getLatestAIModelSize(getLatestAIModelName())
            if(size != ''):
                newText = ("Das ausgewählte AI-Modell wurde für Bilder mit"
                    " einer Auflösung von " + size + "x" + size + " Pixeln trainiert."
                    " Diese Auflösung muss mit der Bad-Pixel-Map und den"
                    " importierten Bildern übereinstimmen.")
                mW.label_descr_resolution.setText(newText)
            elif not getLatestAIModelName() == '':
                newText = ("Das AI-Modell " + getLatestAIModelName() + " ist ausgewählt. Bitte sicherstellen,"
                    " dass die Auflösung der verwendeten Trainingsbilder mit der"
                    " Bad-Pixel-Map und den zu korrigierenden Bildern"
                    " übereinstimmt.")
                mW.label_descr_resolution.setText(newText)
        return

    # get AI model file path
    def getLatestAIModelPath():
        # check for valid directory name:
        if DATA["AI_Model_Pfad"] == '' or not os.path.exists(DATA["AI_Model_Pfad"]):
            return ''

        # check for files in directory:
        name = ''
        numFiles = len(os.listdir(DATA["AI_Model_Pfad"]))
        for i in range(numFiles-1, -1, -1):
            name = DATA["AI_Model_Pfad"] + os.listdir(DATA["AI_Model_Pfad"])[i]
            if name.endswith('.pth'):
                return name
        return ''

    # get AI Model file name
    def getLatestAIModelName():
        # check for valid directory name:
        if DATA["AI_Model_Pfad"] == '' or not os.path.exists(DATA["AI_Model_Pfad"]):
            return ''
        ...
        numFiles = len(os.listdir(DATA["AI_Model_Pfad"]))
        for i in range(numFiles-1, -1, -1):
            name = os.listdir(DATA["AI_Model_Pfad"])[i]
            if name.endswith('.pth'):
                return name
        return ''

    # get pixel dimensions from AI-Model-Filename
    def getLatestAIModelSize(AIModelName):
        if(AIModelName == ''):
            return ''
    ...
    def mW_button_folder_train_img():
        # show folder browser dialog
        if mW.lineEdit_folder_train_img.text() == "" or mW.lineEdit_folder_train_img.text() == " ":
            dirName = widgets.QFileDialog.getExistingDirectory(directory = core.QStandardPaths.
                writableLocation(core.QStandardPaths.DocumentsLocation))
        else:
            dirName = widgets.QFileDialog.getExistingDirectory(directory = mW.lineEdit_folder_train_img.text())

        # Update UI
        mW.lineEdit_folder_train_img.setText(dirName)

        return

    def mW_button_start_training():
        # check for valid parameters
        if (mW.lineEdit_folder_ai.text() == ''):
            # print error message
            openMessageBox(icon=widgets.QMessageBox.Information, 
                text="Wähle zuerst einen Ordner, in dem das AI-Modell gespeichert werden kann!",
                informativeText="",
                windowTitle="Ordner wählen",
                standardButtons=widgets.QMessageBox.Ok,
                pFunction=msgButtonClick)
            return
        if (mW.lineEdit_folder_train_img.text() == ''):
            # print error message
            openMessageBox(icon=widgets.QMessageBox.Information, 
                text="Wähle zuerst einen Ordner mit Trainingsbildern!",
                informativeText="",
                windowTitle="Ordner wählen",
                standardButtons=widgets.QMessageBox.Ok,
                pFunction=msgButtonClick)
            return
        if (mW.lineEdit_mask_num.text() == ''):
            # print error message
            openMessageBox(icon=widgets.QMessageBox.Information, 
                text="Zuerst muss die Anzahl der Masken festgelegt werden, die für das Training verwendet werden soll!",
                informativeText="",
                windowTitle="Fehlende Parameter",
                standardButtons=widgets.QMessageBox.Ok,
                pFunction=msgButtonClick)
            return
        if (mW.lineEdit_num_epoch.text() == ''):
            # print error message
            openMessageBox(icon=widgets.QMessageBox.Information, 
                text="Zuerst muss die Anzahl der Epochen festgelegt werden, die trainiert werden soll!",
                informativeText="",
                windowTitle="Fehlende Parameter",
                standardButtons=widgets.QMessageBox.Ok,
                pFunction=msgButtonClick)
            return
        if (mW.lineEdit_batch_size.text() == ''):
            # print error message
            openMessageBox(icon=widgets.QMessageBox.Information, 
                text="Zuerst muss die Batch Size festgelegt werden, die während dem Training verwendet werden soll!",
                informativeText="",
                windowTitle="Fehlende Parameter",
                standardButtons=widgets.QMessageBox.Ok,
                pFunction=msgButtonClick)
        ...
        # get dimension of training images
        filepathTrainingIm = os.path.join(mW.lineEdit_folder_train_img.text(), os.listdir(mW.lineEdit_folder_train_img.text())[0])
        dimenTraining = np.shape(cv2.imread(filepathTrainingIm))[0]

        # determine resolution: check for previous model: check if
        # resolution of model fits to training images
        latestAIModelSizeStr = getLatestAIModelSize(getLatestAIModelName())
        if (not(latestAIModelSizeStr == '') and not(dimenTraining == int(latestAIModelSizeStr))):
            # print error message
            openMessageBox(icon=widgets.QMessageBox.Information, 
                text="Die Auflösung des zuvor verwendeten Modells passt nicht mit der Auflösung der Trainingsbilder zusammen!",
                informativeText="",
                windowTitle="Ungültige Einstellungen!",
                standardButtons=widgets.QMessageBox.Ok,
                pFunction=msgButtonClick)
            return

        # check if mask folder exists ==> remove folder
        if os.path.exists(os.path.join(mW.lineEdit_folder_train_img.text(),'masks')):
            shutil.rmtree(os.path.join(mW.lineEdit_folder_train_img.text(),'masks'))

        # create new masks folder
        masks_path = os.path.join(mW.lineEdit_folder_train_img.text(),'masks')
        os.mkdir(masks_path)

        # generate required amount of masks
        mask_generation.maskGeneration(dimenTraining, dimenTraining, masks_path, int(mW.lineEdit_mask_num.text()))

        # initialize waiting progressbar
        aiWaitingDialog.progressBar.setValue(0)

        # start training in separate thread
        cfg.trainThreadActive = 1   # enable running/stop flag
        cfg.trainFinishedSuccessful = 0
        TID_TrainProgressThread = threading.Thread(name="trainProgressThread",target=trainProgressThread)
        TID_TrainThread = threading.Thread(name="trainThread",target=train.train,args=(mW.lineEdit_folder_train_img.text(),
            masks_path,                         # masks path
            mW.lineEdit_folder_ai.text(),       # model path
            int(mW.lineEdit_num_epoch.text()),  # number of epochs
            mW.lineEdit_batch_size.text(),      # batch size
            dimenTraining))                     # image resolution
        TID_TrainProgressThread.start()
        TID_TrainThread.start()

        # check for training cancel
        diagResult = aiWaitingDialog.exec()
        if diagResult == widgets.QDialog.Rejected:
            print("Training cancelled")
            # check if thread is alive and stop it
            if TID_TrainThread.is_alive() or TID_TrainProgressThread.is_alive():
                cfg.trainThreadActive = 0
                TID_TrainThread.join()
                TID_TrainProgressThread.join()
        elif diagResult == widgets.QDialog.Accepted and cfg.trainFinishedSuccessful == -1:
            # print error message
                openMessageBox(icon=widgets.QMessageBox.Information, 
                    text="RuntimeError: CUDA out of memory. Try to reduce the batch size.",
                    informativeText="",
                    windowTitle="Runtime Error",
                    standardButtons=widgets.QMessageBox.Ok,
                    pFunction=msgButtonClick)
        return

    def trainProgressThread():
        while cfg.trainThreadActive:
            if cfg.trainFinishedSuccessful == 1:
                aiWaitingDialog.accept()
                return
            elif cfg.trainFinishedSuccessful == -1:
                cfg.trainThreadActive = 0
                aiWaitingDialog.accept()
                return

            aiWaitingDialog.progressBar.setValue(cfg.trainProgress)
    ...
    ### Tab AI-Training
    mW.checkBox_resume_training.stateChanged.connect(mW_checkBox_resume_training)
    mW.button_folder_ai.clicked.connect(mW_button_folder_ai)
    mW.button_folder_train_img.clicked.connect(mW_button_folder_train_img)
    mW.button_start_training.clicked.connect(mW_button_start_training)
    ...
    def KorrekturExportFkt(BPM,F): # Korrigieren
        ...
        if mW.checkBoxAlgorithmusKorrigieren.isChecked():
            ...
            aktuelleZeit = str(datetime.now())[:-7].replace(":","-")    # aktuelle Zeit speichern
            Methode=DATA["Sensors"][int(mW.comboBoxBPMSensor.currentIndex())]["last_korrekturmethode"]
            fortschritt.textEdit.insertPlainText("Bilder werden korrigiert und gespeichert: ")
            ...
            for i in range(np.shape(bildDaten)[0]):
                ...
                if cfg.killFlagThreads==True:
                    return -1
                if mW.radioButtonAlgorithmusNachbar.isChecked():
                    GOOD=np.uint8(correction.nachbar2(bildDaten[i],BPM,Methode,int(eK.labelNachbarFensterbreite.text())))
                elif mW.radioButtonAlgorithmusGradient.isChecked():
                    GOOD=np.uint8(correction.Gradient(bildDaten[i],BPM,Methode,int(eK.labelGradientFensterbreite.text())))
    ...
    # arguments: images_path, BPM_path, model_path, image_resolution
    def KorrekturAIPartialConvolution(images_paths, BPM_path):
        fortschritt.progressBar.setValue(0)
        fortschritt.textEdit.insertPlainText("Bilder werden korrigiert und gespeichert ...")
        
        # get ai-model resolution
        latestAIModelSize = int(getLatestAIModelSize(getLatestAIModelName()))

        # get save paths of all images
        pImagename = [os.path.split(os.path.splitext(images_paths[i])[0])[1] for i in range(len(images_paths))]
        aktuelleZeit = str(datetime.now())[:-7].replace(":","-")    # aktuelle Zeit speichern
        dirName = os.path.join(mW.lineEditSpeicherort.text(), "Korrigierte Bilder vom " + aktuelleZeit)
        if not os.path.exists(dirName):
            os.mkdir(dirName)
        fileNames = [os.path.join(dirName, pImagename[i] +  "_korrigiert.png") for i in range(len(pImagename))]

        test.test(images_paths, 
            BPM_path, 
            mW.lineEdit_folder_ai.text(), 
            latestAIModelSize,
            fileNames)
    ...
    def Prozess(): #Hauptprozess nach Start
        ...
        if FertigFlag:              
            ...
            # start previous version of Korrektur for non-ai-methods
            if mW.radioButtonAlgorithmusNachbar.isChecked() or mW.radioButtonAlgorithmusGradient.isChecked() or mW.radioButtonAlgorithmusNagao.isChecked():
            ...
            elif mW.radioButtonAlgorithmusAIPartialConv.isChecked():
                ThreadID = threading.Thread(name="AI_Partial_Conv", 
                    target=KorrekturAIPartialConvolution, 
                    args=(pathlist, 
                        Speichern.getBPMPath(mW.comboBoxBPMSensor.currentText())))
                ThreadID.start()
