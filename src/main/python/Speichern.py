    ...
    for i in range(len(x)):
        ...
        if DatName.find(Sensor_Name) !=-1:
            ...
            if (Start < 0) or (Ende < 0):   # Wenn die Datei kein V oder . im Namen hat
                ...
                if(Zahl>Nr):
                    Nr=Zahl
    return os.sep.join([dir_path,Sensor_Name+"_V"+str(Nr)+".png"])
...
def Read_json():
    ...
    with open(json_path, "r") as openfile: 
        ...
        # check if AI-Model_Pfad exists in config-file
        if not "AI_Model_Pfad" in json_object:
            # create empty dict element "AI_Model_Pfad" if it doesn't exist yet
            json_object["AI_Model_Pfad"] = ""
